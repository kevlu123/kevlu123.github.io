let defaultCode = `from microbit import *
...
def get_position_from_accelerometer():
    def accelerometer_to_position(acc):
        #sensitivity = 1.25
        #norm_v = acc / 2000 * sensitivity # in range [-1, 1]
        #v = int((norm_v + 1) / 2 * 5)
...
def run_game(ship_list):
    ...
    while True: 
        ...
        if  microphone.sound_level() > 100 and lock_position or button_a.was_pressed():
...
def main():
    while True:
        ...
        #ship_list = [Ship(0, 0, 4, HORIZONTAL_ROTATION)]
        run_game(ship_list)
        game_over()
